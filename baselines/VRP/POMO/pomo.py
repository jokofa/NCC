...
import baselines.VRP.POMO.POMO.cvrp.source.MODEL__Actor.grouped_actors as Actor
from baselines.VRP.POMO.POMO.cvrp.source.utilities import augment_xy_data_by_8_fold
...
def rollout(data: Tuple[Tensor, Tensor, Tensor],
    ...
    t_per_inst = t_total / org_bs
    times = [t_per_inst] * org_bs
    return reward, sols, times


#
def eval_model(data: List[CCPInstance],
               actor: Actor,
               batch_size: int,
               device=torch.device("cpu"),
    ...
    return s_parsed, times


class CVRPDataset(Dataset):
    def __init__(self, data: List[CCPInstance]):
        super(CVRPDataset, self).__init__()
        self.data = [self.make_instance(d) for d in data]

    def make_instance(self, instance: CCPInstance):
