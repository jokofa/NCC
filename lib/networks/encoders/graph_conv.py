...
from lib.networks.utils import get_activation_fn, get_norm


EDGECONVS = [
    # edge_weight:
    "GCNConv", "GCN2Conv", "GraphConv", "GatedGraphConv",
    "TAGConv", "SGConv", "FAConv", "APPNP", "ARMAConv", "APPNP",
    # edge_attr:
...
#
# NOTE: just for the record - we use the following ordering on layers: (Act -> Norm -> Conv -> Res)
#
class GraphConvBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if aggr.lower() == "softmax":
            raise NotImplementedError   # need newest pyg version >= 2.1.0
            aggr = aggr.SoftmaxAggregation(learn=True)
        ...
        self.lin_act = get_activation_fn(activation, module=True, **kwargs) if add_linear else None
    ...
    def forward(self,
        ...
        if self.conv.__class__.__name__ in EDGECONVS:
            ...
            x = self.conv(x, edge_index, edge_weight, **kwargs)
        else:
