...
class VRPCenterEncoder(BaseEncoder):
    ...
    def __init__(self,
        ...
        """
        super(VRPCenterEncoder, self).__init__(input_dim, output_dim, hidden_dim)
    ...
    def create_layers(self, **kwargs):
        ...
        if self.meta_comb_mode == "cat":
            ...
            self.comb_net = nn.Linear(3 * self.hidden_dim, self.hidden_dim)
        else:
        ...
        if self.attn:
            dim = 3 * self.hidden_dim if self.meta_comb_mode == "cat" else self.hidden_dim
            self.attn_net = SAB(
                in_channels=dim,
        ...
        if self.post_proj:
            dim = 3 * self.hidden_dim if (self.meta_comb_mode == "cat" and not self.attn) else self.hidden_dim
            self.post_net = NN(
                in_=dim,
    ...
    def _get_depot_emb(self, node_emb: Tensor) -> Tensor:
        # node_emb is (BS, N+depot, D)
        # first node is always depot
        return self.depot_net(node_emb[:, 0])
    ...
    def _merge_emb(self, c_emb: torch.Tensor, g_emb: torch.Tensor, d_emb: Tensor):
        bs, k, d = c_emb.size()
        g_emb = g_emb[:, None, :].expand(-1, k, -1)
        d_emb = d_emb[:, None, :].expand(-1, k, -1)
        # combine embeddings
        if self.meta_comb_mode == "cat":
            return torch.cat((c_emb, g_emb, d_emb), dim=-1)
        elif self.meta_comb_mode == "proj":
            return self.comb_net(torch.cat((c_emb, g_emb, d_emb), dim=-1))
        else:
            out = self._comb(torch.stack((c_emb, g_emb, d_emb), dim=0), dim=0)
    ...
    def forward(self,
        ...
        # extra encoding of depot before input projection
        d_emb = self._get_depot_emb(node_emb)

        # input proj
        if self.pre_net is not None:
            node_emb = self.pre_net(node_emb)

        c_emb = self._get_center_emb(medoids, node_emb[:, 1:])    # without depot!
        g_emb = self._get_graph_emb(node_emb)
        emb = self._merge_emb(c_emb, g_emb, d_emb)
