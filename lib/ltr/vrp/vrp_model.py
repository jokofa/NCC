...
from lib.networks.encoders import NodeEncoder, VRPCenterEncoder
...
class VRPModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.center_encoder = VRPCenterEncoder(
    ...
    def forward(self,
        ...
        scores = self.decoder(
            ...
            node_emb=node_emb[:, 1:],   # without depot node!
            mask=n_mask,
        )
...
def _test():
    ...
    from lib.ltr.utils import NPZProblemDataset, collate_batch

    PTH = "data/VRP/VRP200/raw/train_mixed_unf0.2_n200_kmax30_s5120_cap1_1_seed1234_labeled_part0.npz"
    BS = 8
    CUDA = False
    SEED = 123
    IDIM = 3
    ...
    data = NPZProblemDataset(npz_file_pth=PTH)
    dl = DataLoader(
        data,
        batch_size=BS,
    ...
    for i, batch in enumerate(dl):
        ...
        x, y = collate_batch(batch, device=device, vrp=True)

        scores, node_emb = model(
            nodes=x.nodes,
